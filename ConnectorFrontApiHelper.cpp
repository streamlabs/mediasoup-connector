	...
	auto func = [](const std::string kind, const std::string producerId, json jsonInput) {
		try {
			if (kind == "audio") {
				...
				try {
					ecodings = jsonInput["encodings"];
				} catch (...) {
				}
				try {
					codecOptions = jsonInput["codecOptions"];
				} catch (...) {
				}
				try {
					codec = jsonInput["codec"];
				} catch (...) {
				}
...
bool ConnectorFrontApiHelper::onConnect(const std::string &clientId, const std::string &transportId, const json &dtlsParameters)
{
	...
	while (MediaSoupInterface::instance().isConnectWaiting() && MediaSoupInterface::instance().isThreadInProgress() &&
	       !MediaSoupInterface::instance().popDataReadyForConnect(dataReady))
		std::this_thread::sleep_for(std::chrono::milliseconds(1));
					...
					std::string &output_value)
{
	...
	while (MediaSoupInterface::instance().isProduceWaiting() && MediaSoupInterface::instance().isThreadInProgress() &&
	       !MediaSoupInterface::instance().popDataReadyForProduce(dataReady))
		std::this_thread::sleep_for(std::chrono::milliseconds(1));
